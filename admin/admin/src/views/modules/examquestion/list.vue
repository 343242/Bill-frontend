<template>
  <div class="main-content">
    ...
    <div v-if="!showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        <el-form-item>
          ...
          <el-button type="warning" icon="el-icon-download" @click="downloadTemplate()" round>下载导入模板</el-button>
          <el-button type="success" icon="el-icon-upload2" @click="batchImportDialogVisible = true" round>批量导入</el-button>
          <el-button
    ...
    <!-- 批量导入对话框 -->
    <el-dialog title="批量导入试题" :visible.sync="batchImportDialogVisible" width="500px" append-to-body>
      <el-form label-width="100px">
        <el-form-item label="选择练习题库" required>
          <el-select v-model="batchImportPaperid" placeholder="请选择练习题库" style="width:100%">
            <el-option
        ...
        <el-form-item label="选择文件" required>
          ...
          >
            ...
            <div slot="tip" class="el-upload__tip">只能上传xls/xlsx文件，请先下载导入模板按格式填写</div>
          </el-upload>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="batchImportDialogVisible = false">取 消</el-button>
        <el-button type="primary" :loading="batchImportLoading" @click="submitBatchImport">确认导入</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  computed: {
    batchImportAction() {
      return `/${base.get().name}/${this.$api.examquestionbatchimport}`;
    },
    batchImportData() {
      return {
        paperid: this.batchImportPaperid
      };
    }
  },
  methods: {
    ...
    downloadTemplate() {
      window.location.href = `/${base.get().name}/${this.$api.examquestiontemplate}`;
    },
    loadPaperOptions() {
      this.$http({
    ...
    submitBatchImport() {
      if (!this.batchImportPaperid) {
        this.$message.warning("请先选择练习题库");
        return;
      }
    ...
    handleBatchImportSuccess(response, file, fileList) {
      ...
      if (response && response.code === 0) {
        ...
        this.batchFileList = [];
        this.batchImportPaperid = "";
        this.search();
      } else {
        this.$message.error(response.msg || "导入失败");
      }
    },
    handleBatchImportError(err, file, fileList) {
      this.batchImportLoading = false;
